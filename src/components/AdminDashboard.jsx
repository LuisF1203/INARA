...
import { FaBeer, FaProductHunt } from 'react-icons/fa'; // Importa el ícono que deseas usar
...
function AdminDashboard(){
            ...
            try {
                ...
                const ordersData = querySnapshot.docs.map(doc => {
                    ...
                    const orderStatusColor = {
                        "in process": "#60a5fa",  // Azul claro para "en proceso"
                        "created": "#f59e0b",     // Naranja para "creado"
                        "payed": "#facc15",       // Amarillo para "pagado"
                        "send": "#a78bfa",        // Púrpura claro para "enviado"
                        "received": "#4ade80"     // Verde claro para "recibido"
                    };
                    
                    
                    const width = orderStatusWidth[(data.status).toLowerCase()];
                    const color = orderStatusColor[(data.status).toLowerCase()];
                    return { 
                        id: doc.id,
                        ...data,
                        width, // Agrega el ancho basado en el estado,
                        color
                    };
                });
    ...
    const handleSendOrder=(el)=>{
        el.preventDefault();
        alert("enviando producto")
    }
    ...
    const handleRangeChange = (event) => {
        ...
        updateIconPosition(event.target.value);
        if(event.target.value==100){
            alert("yeah")
        }
    };

    const updateIconPosition = (value) => {
        const percentage = value / 100;
        const width = rangeRef.current.offsetWidth;
        const newPosition = width * percentage;
        setIconPosition(newPosition);
    };

    useEffect(() => {
        updateIconPosition(rangeValue);
    }, [rangeValue]);




    const handleRangeRelease = (event) => {
        if(rangeValue<100){
            event.target.value=0;
            event.target.style.background="rgba(0, 255, 0, 0.2)";
            updateIconPosition(10);
        }
    };

    // Calcula el color en función del valor actual
    const getColor = (value) => {
        // La opacidad comienza en 0.2 y aumenta hasta 1
        const opacity = 0.2 + (value / 100) * 0.8;
        return `rgba(0, 255, 0, ${opacity})`; // R: 0, G: 255, B: 0, A: de 0.2 a 1
    };
    
    

    const thumbImageUrl = "https://example.com/your-image.png"; // Reemplaza con la URL de tu imagen
        ...
        <div class="min-h-screen bg-gray-50/50">
            ...
            <aside class="bg-gradient-to-br from-gray-800 to-gray-900 -translate-x-80 fixed inset-0 z-50 my-4 ml-4 h-[calc(100vh-32px)] w-72 rounded-xl transition-transform duration-300 xl:translate-x-0">
                ...
                <button class="middle none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-8 max-w-[32px] h-8 max-h-[32px] rounded-lg text-xs text-white hover:bg-white/10 active:bg-white/30 absolute right-0 top-0 grid rounded-br-none rounded-tl-none xl:hidden" type="button">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2.5" stroke="currentColor" aria-hidden="true" class="h-5 w-5 text-white">
                ...
                <ul class="mb-4 flex flex-col gap-1">
                    ...
                    <li>
                    <a class="" onClick={handleSignOut} href="#">
                        <button class="middle none font-sans font-bold center transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 rounded-lg text-white hover:bg-white/10 active:bg-white/30 w-full flex items-center gap-4 px-4 capitalize" type="button">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" class="w-5 h-5 text-inherit">
            ...
            <div class="p-4 xl:ml-80">
                ...
                <div class="flex flex-col-reverse justify-between gap-6 md:flex-row md:items-center">
                    ...
                    <button class="relative middle none font-sans font-medium text-center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none w-10 max-w-[40px] h-10 max-h-[40px] rounded-lg text-xs text-gray-500 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30 grid xl:hidden" type="button">
                        <span class="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" strokeWidth="3" class="h-6 w-6 text-blue-gray-500">
                    ...
                    <a href="#">
                        <div class="middle none font-sans font-bold center uppercase transition-all disabled:opacity-50 disabled:shadow-none disabled:pointer-events-none text-xs py-3 rounded-lg text-gray-500 hover:bg-blue-gray-500/10 active:bg-blue-gray-500/30 hidden items-center gap-1 px-4 xl:flex" type="button">
                ...
                <div class="mb-12 grid gap-y-10 gap-x-6 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div class="border-t border-blue-gray-50 p-4">month
                        <p class="block antialiased font-sans text-base leading-relaxed font-normal text-blue-gray-600">
                ...
                <div class="mb-4 grid grid-cols-1 gap-6 xl:grid-cols-3">
                    ...
                    <div class="relative bg-clip-border rounded-xl overflow-hidden bg-transparent text-gray-700 shadow-none m-0 flex items-center justify-between p-6">
                        <div>
                        <h6 class="block antialiased tracking-normal font-sans text-base font-semibold leading-relaxed text-blue-gray-900 mb-1">Orders</h6>
                        <p class="antialiased font-sans text-sm leading-normal flex items-center gap-1 font-normal text-blue-gray-600">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="3" stroke="currentColor" aria-hidden="true" class="h-4 w-4 text-blue-500">
                        ...
                        <span class="absolute top-1/2 left-1/2 transform -translate-y-1/2 -translate-x-1/2">
                            <IoReload onClick={()=>fetchOrdersData()} className="w-4 h-4 hover:animate-spin" />
                        </span>
                        </button>
                    </div>
                    <div class="p-6 overflow-x-scroll px-0 pt-0 pb-2">
                        ...
                        <thead>
                            ...
                            <th class="border-b border-blue-gray-50 py-3 px-6 text-left">
                                <p class="block antialiased font-sans text-[11px] font-medium uppercase text-blue-gray-400">users</p>
                            </th>
                            <th class="border-b border-blue-gray-50 py-3 px-6 text-left">
                        ...
                        {orders.map(order => (
                                ...
                                <tr onClick={()=> {
                                    ...
                                    document.getElementById('orderInfoForm').fName.value=order.firstName;
                                    document.getElementById('orderInfoForm').lName.value=order.lastName;
                                    document.getElementById('orderInfoForm').email.value=order.email;
                                    document.getElementById('orderInfoForm').phone.value=order.phone;
                                    document.getElementById('orderInfoForm').address.value=order.address;
                                    document.getElementById('orderInfoForm').city.value=order.city;
                                    document.getElementById('orderInfoForm').zip.value=order.zipCode;
                                    document.getElementById('orderInfoForm').inst.value=order.instructions;
                                        ...
                                        <div class="flex flex-wrap -mx-3">
                                            <div class="w-full px-3 sm:w-1/2">
                                                <div class="mb-5">
                                                    ...
                                                    <input type="text" value="${product.id}" name="uId" id="uId" disabled
                                                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                                                </div>
                                            </div>
                                            <div class="w-full px-3 sm:w-1/2">
                                                <div class="mb-5">
                                                    <label for="prQuantity" class="mb-3 block text-base font-medium text-[#07074D]">
                                                        Quantity
                                                    </label>
                                                    <input type="number" value="${product.quantity}" name="prQuantity" id="prQuantity" disabled
                                                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                                                </div>
                                            </div>
                                            <div class="w-full px-3 sm:w-1/2">
                                                <div class="mb-5">
                                                    <label for="prName" class="mb-3 block text-base font-medium text-[#07074D]">
                                                        Name
                                                    </label>
                                                    <input type="text" value="${product.name}" name="prName" id="prName" disabled
                                                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                                                </div>
                                            </div>
                                            <div class="w-full px-3 sm:w-1/2">
                                                <div class="mb-5">
                                                    <label for="prColor" class="mb-3 block text-base font-medium text-[#07074D]">
                                                        Color
                                                    </label>
                                                    <input type="text" value="${product.color}" name="prColor" id="prColor" disabled
                                                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                                                </div>
                                            </div>
                                            <div class="w-full px-3 sm:w-1/2">
                                            <div class="mb-5">
                                                    <label for="prPrice" class="mb-3 block text-base font-medium text-[#07074D]">
                                                        Price
                                                    </label>
                                                    <input type="number" value="${product.price}" name="prPrice" id="prPrice" disabled
                                                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                                                </div>
                                            </div>
                                        </div>
                                ...
                                }}  className="cursor-pointer hover:bg-slate-50" key={order.id}>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <div class="flex items-center gap-4">
                                    <p class="block antialiased font-sans text-sm leading-normal text-blue-gray-900 font-bold">{order.email}</p>
                                    </div>
                                </td>

                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <p class="block antialiased font-sans text-xs font-medium text-blue-gray-600">${order.totalPrice}</p>
                                </td>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <div class="w-10/12">
                                    <p class="antialiased font-sans mb-1 block text-xs font-medium text-blue-gray-600">{order.status}</p>
            ...
            <div class="flex items-center justify-center p-12">
              <div class="mx-auto w-full max-w-[550px]">
                <h2 className="text-2xl font-bold mb-5">Order info</h2>
                <button onClick={()=>document.getElementById('my_modal_2').close()} className="absolute right-5 top-5">
                      <IoMdClose />
                </button>
                <form id="orderInfoForm" onSubmit={(el)=>handleSendOrder(el)} method="POST">
                
                
                <div id="products">

                </div>
                <hr className="p-[1px] bg-[#e9e9e9] mt-2 mb-2" />
                <h2 className="text-2xl font-bold mb-5">Deliver info</h2>               
                <div id="mainContentModal">
                    ...
                    <div class="w-full px-3 sm:w-1/2">
                        ...
                        <input type="text" name="fName" id="fName" disabled placeholder="Nombre(s)"
                            class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                        </div>
                    </div>
                    <div class="w-full px-3 sm:w-1/2">
                        ...
                        <input type="text" name="lName" id="lName" disabled placeholder="Apellidos"
                            class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                        </div>
                    </div>
                    </div>
                    ...
                    <input type="email" name="email" id="email" disabled placeholder="Email"
                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                    </div>
                    <div class="mb-5">
                    ...
                    <input type="tel" name="phone" id="phone" disabled placeholder="Número de Teléfono"
                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                    </div>
                    <div class="mb-5">
                    ...
                    <input type="text" name="address" id="address" disabled placeholder="Dirección completa"
                        class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                    </div>
                    <div class="flex flex-wrap -mx-3">
                    <div class="w-full px-3 sm:w-1/2">
                        ...
                        <input type="text" name="city" id="city" disabled placeholder="Ciudad"
                            class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                        </div>
                    </div>
                    <div class="w-full px-3 sm:w-1/2">
                        ...
                        <input type="text" name="zip" id="zip" disabled placeholder="Código Postal"
                            class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md" />
                        </div>
                    </div>
                    <div class="w-full px-3">
                        ...
                        <textarea  disabled name="inst" id="" cols="30" rows="2" maxLength="100" class="w-full rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"></textarea>
                        </div>
                    </div>
                    </div>
                    <div> 
                    ...
                    <SlideButton
                        ...
                        caret={
                        <>
                            <LuChevronsRight size={40} color="white"/>
                        </> }
                    ...
                    <div className="relative" style={{height: "50px"}}>
                        ...
                        <input 
                            ref={rangeRef}
                            className="absolute z-10 w-full h-2 bg-gray-200 rounded-3xl appearance-none cursor-pointer" 
                            type="range" 
                            min="0" 
                            max="100" 
                            value={rangeValue} 
                            onChange={handleRangeChange}
                            onMouseUp={handleRangeRelease} // Para dispositivos de escritorio
                            onTouchEnd={handleRangeRelease} // Para dispositivos táctiles

                            defaultValue="20%"
                            style={{ WebkitAppearance: 'none', height: "50px",background: getColor(rangeValue)}}
                        />
                        <div className="absolute inset-0 flex justify-center items-center">
                            <span className="text-sm font-medium text-black">Desliza para enviar</span>
                        </div>
                    </div>
                ...
                input[type='range']::-webkit-slider-thumb {
                    ...
                    width: 20px;
                    height: 20px;
                    background: transparent; /* Color blanco */
                    cursor: pointer;
                    border-radius: 50%;
                }
